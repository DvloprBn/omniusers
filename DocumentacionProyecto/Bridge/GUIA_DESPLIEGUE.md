...
> **Hallazgo real (proyecto Espiral, 2026-08-27): reconstruir sin subir el código nuevo primero solo reconstruye el código VIEJO que ya estaba ahí.** El paso 1 (subir el código — `scp`, o `rsync` de todo el repo si así se está trabajando) no es opcional ni "solo si no se te olvidó" — es el ÚNICO paso que de verdad copia algo nuevo al servidor. `docker compose up -d --build` reconstruye lo que sea que YA esté en `RUTA_PROYECTO`, sin importar qué tan reciente sea el código en la máquina local. **Síntoma real de este error exacto**: un cambio real (un campo nuevo en la respuesta de una API) se prueba en local, se sube a producción "reconstruyendo" — pero como el paso 1 nunca se corrió, el sitio real sigue sirviendo la versión vieja, y si el FRONTEND sí se actualizó en otro momento pero el BACKEND no (o viceversa), el resultado es un error real en producción (`Cannot read properties of undefined`) porque un lado del sitio ya espera un campo que el otro lado todavía no manda. Si hay duda de si el paso 1 se corrió de verdad la última vez, mejor volver a correrlo — es gratis (rsync/scp no rompen nada si el archivo ya estaba igual) y elimina la duda por completo.
