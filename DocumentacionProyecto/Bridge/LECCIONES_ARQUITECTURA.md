...
**Costo real de "arreglarlo bien" vs. beneficio real**: forzar CORS real en cada `<img>` (agregar `crossorigin="anonymous"` a cada uso, y confirmar que el CDN responde con `Access-Control-Allow-Origin` compatible con eso) es un cambio que toca potencialmente decenas de lugares en el frontend — y el beneficio real de COEP (habilitar aislamiento cross-origin para APIs sensibles como `SharedArrayBuffer`/temporizadores de alta precisión) casi nunca aplica a un sitio de comercio/contenido normal. Un escáner de seguridad (securityheaders.com y similares) lista COEP como "Upcoming Header" — no resta puntos del grado principal — así que no hay urgencia real de agregarlo si el costo no se justifica. `Cross-Origin-Resource-Policy`, por su parte, no depende de terceros (controla quién puede cargar TUS PROPIOS recursos) — seguro de agregar sin más investigación.

**Corrección real (2026-08-27, el mismo día — encontrado probando un pago real de punta a punta, no solo grep del código propio): `Cross-Origin-Opener-Policy: same-origin` SÍ puede depender de terceros, y la verificación original de esta lección estaba incompleta.** `COOP:same-origin` rompió un checkout real de Stripe — el formulario de tarjeta quedaba completamente vacío, una petición real de Stripe (`hcaptcha-invisible`, su propio mecanismo anti-fraude) se bloqueaba. Causa real: Stripe.js (y probablemente cualquier SDK de pago con verificación anti-fraude/3D Secure/inicio de sesión guardado, ej. "Link") usa `window.opener`/popups INTERNAMENTE — invisible a un grep de `window.open` en el código propio del proyecto, porque vive dentro del script de terceros, no en código propio. **La regla correcta**: buscar `window.open` en el código propio SOLO descarta que EL PROYECTO MISMO rompa algo — nunca descarta que un SDK de terceros (pagos, mapas con autenticación, video con licencias DRM, cualquier cosa con su propio flujo de ventana) dependa de esos mecanismos por dentro. La única verificación real es probar el flujo completo de punta a punta con la cabecera puesta. Si el sitio procesa pagos reales (Stripe, PayPal, Mercado Pago, cualquiera), usar `same-origin-allow-popups` en vez de `same-origin` a propósito — mantiene la protección real (aísla de ventanas que ABREN el sitio) sin romper las ventanas que el sitio o sus SDKs de terceros abren ellos mismos; confirmar con un pago de prueba real después de fijarlo.
